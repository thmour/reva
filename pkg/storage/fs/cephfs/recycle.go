...
func (fs *cephfs) ListRecycle(ctx context.Context) (items []*provider.RecycleItem, err error) {
...
func (fs *cephfs) RestoreRecycleItem(ctx context.Context, key string) (err error) {
	log := appctx.GetLogger(ctx)
...
func (fs *cephfs) PurgeRecycleItem(ctx context.Context, key string) (err error) {
	log := appctx.GetLogger(ctx)
...
func (fs *cephfs) getRecycleRoot(ctx context.Context) string {
	if fs.o.EnableHome {
		u := user.ContextMustGetUser(ctx)
		// TODO use layout, see Tree.Delete() for problem

...
func initializeDB(root, dbName string) (*sql.DB, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "cephfs: error opening DB connection")
	}

	stmt, err := db.Prepare("CREATE TABLE IF NOT EXISTS recycled_entries (key TEXT PRIMARY KEY, path TEXT)")
	if err != nil {
		return nil, errors.Wrap(err, "cephfs: error preparing statement")
	}
	_, err = stmt.Exec()
	if err != nil {
		return nil, errors.Wrap(err, "cephfs: error executing create statement")
	}
...
func (fs *cephfs) addToRecycledDB(ctx context.Context, key, fileName string) error {
	stmt, err := fs.db.Prepare("INSERT INTO recycled_entries VALUES (?, ?)")
	if err != nil {
		return errors.Wrap(err, "cephfs: error preparing statement")
	}
	_, err = stmt.Exec(key, fileName)
	if err != nil {
		return errors.Wrap(err, "cephfs: error executing insert statement")
	}
	return nil
}

func (fs *cephfs) getRecycledEntry(ctx context.Context, key string) (string, error) {
	var filePath string
...
func (fs *cephfs) removeFromRecycledDB(ctx context.Context, key string) error {
	...
	_, err = stmt.Exec(key)
	if err != nil {
		return errors.Wrap(err, "cephfs: error executing delete statement")
	}
	return nil
}

func (fs *cephfs) addToACLDB(ctx context.Context, resource, grantee, role string) error {
...
func (fs *cephfs) getACLs(ctx context.Context, resource string) (*sql.Rows, error) {
...
func (fs *cephfs) removeFromACLDB(ctx context.Context, resource, grantee string) error {
...
func (fs *cephfs) addToFavoritesDB(ctx context.Context, resource, grantee string) error {
...
func (fs *cephfs) removeFromFavoritesDB(ctx context.Context, resource, grantee string) error {
...
func (fs *cephfs) addToMetadataDB(ctx context.Context, resource, key, value string) error {
...
func (fs *cephfs) removeFromMetadataDB(ctx context.Context, resource, key string) error {
...
func (fs *cephfs) getMetadata(ctx context.Context, resource string) (*sql.Rows, error) {
...
func (fs *cephfs) addToReferencesDB(ctx context.Context, resource, target string) error {
...
func (fs *cephfs) getReferenceEntry(ctx context.Context, resource string) (string, error) {
	var target string
